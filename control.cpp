...
GravityController::GravityController(game_object * go_, std::list<game_object*>* go_list_ ):go(go_), go_list(go_list_) {
	assert(go);
	assert(go_list);
}
	
void GravityController::apply(){
	std::list<game_object*>& ref_go_list = *go_list;

	for(std::list<game_object*>::iterator it = ref_go_list.begin();it != ref_go_list.end(); it++){
		//if (  (*it)->mass() < 10 * go->mass()) continue;
		float DistSq = pow(go->posx() - (*it)->posx(),2) + pow(go->posy() - (*it)->posy(),2) + pow(go->posz() - (*it)->posz(),2);
		if (DistSq < 2.0) continue;
		if (DistSq > 5000.0) continue;
		float G = ( go->mass() * (*it)->mass() ) /  DistSq;
		if(G>10.0f) G = 10.0f;
		vector_2 DirF = -1.0f * vector_2( go->posx() - (*it)->posx(), go->posy() - (*it)->posy()) / sqrt(DistSq);
